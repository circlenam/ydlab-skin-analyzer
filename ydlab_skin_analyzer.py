...
st.set_page_config(page_title="YD Lab 피부·두피 분석", page_icon="🔬", layout="centered")
...
.hero { background: linear-gradient(135deg, #0f3460 0%, #1a5276 100%);
...
.card { background:white; border-radius:12px; padding:1.4rem; margin-bottom:1rem;
...
.step-badge { display:inline-flex; align-items:center; justify-content:center;
    ...
    background:#0f3460; color:white; font-size:0.72rem; font-weight:700; margin-right:0.5rem; flex-shrink:0; }
.coupang-btn { display:inline-block; background:#ff6b35; color:white;
    padding:0.3rem 0.8rem; border-radius:6px; font-size:0.75rem; font-weight:600; text-decoration:none; margin-left:0.5rem; }
.partners-notice { background:#fff8f0; border:1px solid #ffcc80; border-radius:8px;
    padding:0.6rem 1rem; font-size:0.72rem; color:#e65100; margin-top:0.8rem; line-height:1.6; }
...
.calc-btn-wrap { margin-top:0.8rem; }
.calc-btn-link { display:block; width:100%; padding:0.85rem;
    background:linear-gradient(135deg,#1565c0,#1976d2);
    color:white; text-align:center; border-radius:10px;
    font-weight:700; font-size:0.92rem; text-decoration:none;
    box-shadow:0 2px 8px rgba(21,101,192,0.25); }
.calc-btn-link:hover { opacity:0.9; }
</style>
...
STATION_CANDIDATES = {
    ...
    "연수구":["연수","송도","동춘","옥련"], "남동구":["구월","남동","논현","구월동"],
    "안산":["본오동","고잔동","부곡동1","선부동"], "시흥":["정왕동","대야동","배곧동","목감동"],
    "서울":["중구","종로구"], "기타":["중구"],
}
...
def get_concentration(ingredient, skin_type):
    ...
    D = {
        "히알루론산":("1~2%" if not is_s else "0.5~1%","저·고분자 혼합 권장"),
        "세라마이드":("1~3%" if not is_s else "0.5~1%","NP·AP·EOP 3종 혼합"),
        "나이아신아마이드":("5~10%" if not is_s else "2~5%","10% 초과 시 자극 가능"),
        "레티놀":("0.1~0.3%" if not is_s else "0.05%","야간 전용·서서히 농도↑"),
        "비타민C":("10~15%" if not is_s else "5~10%","pH 3.5 이하 유지"),
        "비타민C 유도체":("3~5%","아스코빌글루코사이드"),
        "펩타이드":("2~5%" if not is_s else "1~3%","아세틸헥사펩타이드-3"),
        "판테놀":("1~5%" if not is_s else "1~2%","Pro-비타민B5"),
        "살리실산":("0.5~2%","두피 각질 용해"),
...
def _find_cp(ing):
    ...
    for k in COUPANG_LINKS:
        if k in ing or ing in k: return COUPANG_LINKS[k]
    return {"url":f"https://www.coupang.com/np/search?q={ing.replace(' ','+')}+원액","name":ing}

def _cp_link_tag(ing, fs="8px"):
    url = _find_cp(ing).get("url","")
    return f'<a href="{url}" target="_blank" style="color:#ff6b35;font-size:{fs};text-decoration:none;">🛒</a>' if url else ""
...
# 계산기 URL 생성
# ══════════════════════════════════════════════════════════════
def generate_calculator_url(mixing: dict, total_vol: int = 30) -> str:
    """혼합 가이드 딕셔너리를 받아 calculator.html URL을 생성합니다."""
    params = []
    for i, (name, ratio) in enumerate(mixing["ratios"].items(), 1):
        target = INGREDIENT_TARGET_PCT.get(name, 1.0)
        vol_ml = mixing["ml"].get(name, 0)
        params += [
            f"s{i}={quote(name)}",
            f"p{i}={target}",
            f"r{i}={ratio}",
    ...
    params.append(f"vol={total_vol}")
    return CALCULATOR_URL_BASE + "?" + "&".join(params)
...
def calc_ceei(pm25_avg, residence_years):
    ...
    if ceei < 50:
        return (ceei,"낮음",f"<span class='chip chip-good'>CEEI {ceei} [낮음]</span>","환경 노출 영향 낮음 — 기본 보습·자외선 차단 유지")
    elif ceei < 150:
        return (ceei,"보통",f"<span class='chip chip-mid'>CEEI {ceei} [보통]</span>","중간 수준 환경 노출 — 항산화 성분 정기적 사용 권장")
    elif ceei < 300:
        return (ceei,"높음",f"<span class='chip chip-warn'>CEEI {ceei} [높음]</span>","높은 환경 노출 누적 — 항산화·장벽강화 집중 케어 필요")
    else:
...
def generate_mixing_guide(ingredients, skin_type, ceei_grade, total_ml=20):
    BW = {"히알루론산":35,"세라마이드":20,"나이아신아마이드":20,"판테놀":25,"비타민C":15,
...
def generate_scalp_mixing_guide(ingredients, scalp_result, seei_grade, total_ml=30):
    BW = {"징크피리치온":25,"살리실산":20,"살리실산(BHA)":20,"바이오틴":20,
          "판테놀 (두피용)":25,"판테놀":25,"나이아신아마이드":15,"비타민C":10,
          "비타민C 유도체":10,"히알루론산":15,"세라마이드":10,
          "티트리 오일":15,"티트리오일":15,"티트리":15,"로즈마리 오일":10,"멘톨":5,"소듐PCA":15}
    ...
    def iss(i): return i in {"살리실산","살리실산(BHA)","BHA"}
    def ist(i): return i in {"티트리 오일","티트리오일","티트리","Tea Tree"}
...
def show_mixing_card(mixing, title, is_scalp=False):
    ...
    st.markdown(f"<div class='card-label' style='color:#1a5276;'>{title}</div>",
                unsafe_allow_html=True)
    ...
    for ing, pct in sorted(mixing["ratios"].items(), key=lambda x: -x[1]):
        ...
        url = _find_cp(ing).get("url", "")
        btn = (f"<a href='{url}' target='_blank' class='coupang-btn'>🛒 구매</a>"
               if url else "")
        rows += (
            f"<div class='mixing-row'>"
            f"<span class='mixing-ing'>{ing}{btn}</span>"
            f"<div class='mixing-bar-wrap'>"
            f"<div class='{bc}' style='width:{pct}%;'></div></div>"
            f"<span class='mixing-pct'>{pct}%</span>"
            f"<span class='mixing-ml'>{ml}ml</span></div>"
        )
    ...
    # 쿠팡 파트너스 고지
    st.markdown(f"<div class='partners-notice'>{COUPANG_DISCLAIMER}</div>",
                unsafe_allow_html=True)

    # ── 계산기 연동 버튼 ──────────────────────────────────────
    calc_url = generate_calculator_url(mixing, mixing["total_ml"])
    st.markdown(
        f"""<div class='calc-btn-wrap'>
          <a href="{calc_url}" target="_blank" class="calc-btn-link">
            🧮 내 제품 농도로 정확한 혼합량 계산하기
          </a>
          <div style='font-size:0.72rem;color:#888;text-align:center;margin-top:0.4rem;'>
            구매한 제품의 농도가 다를 경우 → 계산기에서 실제 투입량을 확인하세요
          </div>
        </div>""",
        unsafe_allow_html=True)
...
def show_skin_result(result, air, region, residence_years_str, participant_id,
  ...
  <div style='display:flex;align-items:center;gap:1.5rem;flex-wrap:wrap;'>
    ...
    ing_html = "".join([
        f"<span class='ingredient-chip'>{ing} "
        f"{_cp_link_tag(ing,fs='0.7rem')}</span>"
        for ing in ings])
    st.markdown(
...
def show_scalp_result(result, air, region, residence_years_str, participant_id,
  ...
  <div style='display:flex;align-items:center;gap:1.5rem;flex-wrap:wrap;'>
    ...
    ing_html = "".join([
        f"<span class='scalp-ingredient-chip'>{ing} "
        f"{_cp_link_tag(ing,fs='0.7rem')}</span>"
        for ing in ings])
    st.markdown(
...
def _mixing_html(mixing, color):
    ...
    rows = "".join([
        ...
        f"border-bottom:1px solid #f0f0f0;font-size:9px;'>"
    ...
    steps = "".join([
        f"<div style='display:flex;align-items:center;gap:6px;padding:4px 0;"
        f"font-size:9px;border-bottom:1px solid #f0f0f0;'>"
    ...
    return (
        f"<div class='section'>"
        f"<div class='stitle'>맞춤 혼합 비율 (총 {mixing['total_ml']}ml)</div>"
        f"{rows}"
        f"<div style='margin-top:8px;font-size:8px;font-weight:700;"
        f"color:{color};margin-bottom:5px;'>제조 순서</div>{steps}"
        f"<div style='font-size:7px;color:#e65100;padding:5px 8px;"
        f"background:#fff8f0;border-radius:4px;margin-top:6px;'>"
        f"{COUPANG_DISCLAIMER}</div></div>")
...
def generate_skin_report_html(result, air, region, residence_years,
    ...
    ing_h = "".join([
        ...
        f"display:inline-block;margin:2px;'>{i} {_cp_link_tag(i)}</span>"
        for i in ings])
...
def generate_skin_order_html(result, air, region, residence_years,
    ...
    rows = "".join([
        ...
        f"{mixing['ratios'].get(ing,'-') if mixing else '-'}%"
        f" ({mixing['ml'].get(ing,'-') if mixing else '-'}ml)</td>"
        f"<td style='padding:8px;border:1px solid #e4e8ee;font-family:monospace;'>"
        f"{get_concentration(ing,st_)[0]}</td>"
        f"<td style='padding:8px;border:1px solid #e4e8ee;'>{_cp_buy_btn(ing)}</td>"
        f"<td style='padding:8px;border:1px solid #e4e8ee;color:#888;font-size:9px;'>"
        f"{get_concentration(ing,st_)[1]}</td></tr>"
  ...
  <div style="font-size:9px;opacity:0.6;margin-top:3px;">
    AI 피부 분석 기반 맞춤형 화장품 제조 요청</div></div>
...
<div style="margin-bottom:16px;">
  ...
  {steps_h}
  <div style="font-size:8px;color:#e65100;padding:6px 8px;
    background:#fff8f0;border-radius:4px;margin-top:8px;">{COUPANG_DISCLAIMER}</div>
</div>
...
def generate_scalp_order_html(result, air, region, residence_years,
    ...
    pm = {
        ...
        "티트리오일":"두피 항균·항염·진정","티트리":"두피 항균·항염·진정",
    ...
    rows = "".join([
        ...
        f"font-weight:600;color:#1b5e20;'>{i+1}</td>"
        ...
        f"{mixing['ratios'].get(ing,'-') if mixing else '-'}%"
        f" ({mixing['ml'].get(ing,'-') if mixing else '-'}ml)</td>"
        f"<td style='padding:8px;border:1px solid #a5d6a7;font-family:monospace;color:#1b5e20;'>"
        f"{get_concentration(ing,st_)[0]}</td>"
        f"<td style='padding:8px;border:1px solid #a5d6a7;'>{_cp_buy_btn(ing)}</td>"
        f"<td style='padding:8px;border:1px solid #a5d6a7;color:#888;font-size:9px;'>"
        f"{get_concentration(ing,st_)[1]}</td></tr>"
        for i, ing in enumerate(ings)])
    steps_h = "".join([
        f"<div style='display:flex;align-items:center;gap:8px;padding:5px 0;"
...
<div style="margin-bottom:16px;border:1.5px solid #a5d6a7;border-radius:10px;overflow:hidden;">
  ...
  <div style="padding:12px 14px;background:#f0faf4;">
    <div style="display:grid;grid-template-columns:22px 1fr;gap:5px 8px;
      ...
      <span>샴푸 후 타월로 두피 물기를 가볍게 눌러서 제거해 주세요
        <span style="color:#888;">(완전 건조 X — 살짝 촉촉한 상태가 흡수에 유리)</span></span>
      <span style="background:#1b5e20;color:white;border-radius:50%;width:20px;height:20px;
        display:inline-flex;align-items:center;justify-content:center;
        font-size:9px;font-weight:700;">2</span>
      <span>세럼을 두피에 직접 소량씩 도포합니다
        <span style="color:#888;">(1회 약 1~2ml, 두피 전체에 고르게)</span></span>
      <span style="background:#1b5e20;color:white;border-radius:50%;width:20px;height:20px;
        display:inline-flex;align-items:center;justify-content:center;
        font-size:9px;font-weight:700;">3</span>
      <span>손가락 끝으로 두피를 가볍게 원형 마사지해 주세요
        <span style="color:#888;">(1~2분, 혈행 촉진 효과)</span></span>
      <span style="background:#1b5e20;color:white;border-radius:50%;width:20px;height:20px;
        display:inline-flex;align-items:center;justify-content:center;
        font-size:9px;font-weight:700;">4</span>
      <span>씻어내지 않고 그대로 건조합니다
        <span style="color:#888;">(미지근한 바람 권장, Leave-on 타입)</span></span>
      <span style="background:#1b5e20;color:white;border-radius:50%;width:20px;height:20px;
        display:inline-flex;align-items:center;justify-content:center;
        font-size:9px;font-weight:700;">5</span>
      <span>매일 또는 격일 사용을 권장합니다</span>
    </div>
    <div style="display:grid;grid-template-columns:1fr 1fr;gap:6px;font-size:9px;">
      <div style="background:white;border:1px solid #c8e6c9;border-radius:6px;padding:6px 8px;">
        <div style="font-weight:700;color:#1b5e20;margin-bottom:3px;">⏰ 사용 시점</div>
        <div style="color:#555;">샴푸 후 즉시 사용<br>취침 전 사용 시 효과 극대화</div>
      </div>
      <div style="background:white;border:1px solid #c8e6c9;border-radius:6px;padding:6px 8px;">
        <div style="font-weight:700;color:#1b5e20;margin-bottom:3px;">📦 보관 방법</div>
        <div style="color:#555;">직사광선 피해 서늘한 곳 보관<br>개봉 후 3개월 내 사용 권장</div>
      </div>
      <div style="background:white;border:1px solid #c8e6c9;border-radius:6px;padding:6px 8px;">
        <div style="font-weight:700;color:#1b5e20;margin-bottom:3px;">✅ 사용 전 확인</div>
        <div style="color:#555;">패치 테스트 권장<br>팔 안쪽에 소량 도포 후 24시간 확인</div>
      </div>
      <div style="background:#fff8f0;border:1px solid #ffcc80;border-radius:6px;padding:6px 8px;">
        <div style="font-weight:700;color:#e65100;margin-bottom:3px;">⚠️ 주의사항</div>
        <div style="color:#555;">이상 반응(가려움·홍조) 시 즉시 중단<br>눈에 들어가지 않도록 주의</div>
      </div>
    </div>
  </div>
</div>"""
  ...
  <div style="font-size:9px;opacity:0.6;margin-top:3px;">
    AI 두피 분석 + SEEI v3 기반 맞춤형 두피케어 제조 요청</div></div>
  <div style="font-family:monospace;background:rgba(255,255,255,0.15);
    padding:4px 10px;border-radius:4px;">{code}</div>
</div>
<div class="body">
...
행사장에서 받은 이벤트 코드를 입력해 주세요.</p></div>""",
    ...
    kma_status = ("✅ 기상청 API 연동" if kma_key
                  else "⚠️ 기상청 API 미연동 (모의값 사용)")

    st.markdown(f"""<div class='hero'>
<div class='hero-label'>YD Lab · 재능대학교 AI-바이오분석특화연구소</div>
<h1>🔬 AI 피부·두피 분석 v3.3</h1>
...
<p style='font-size:0.75rem;opacity:0.6;margin-top:0.5rem;'>{kma_status}</p>
    ...
    if mode == "skin":
        ...
        st.info("💆 **두피 분석 모드** — 두피 6지표 + SEEI v3 (PM2.5·PM10·NO₂·O₃·UV·습도)")
    else:
