...
두피 3지표: scalp_keratin_score(각질 없을수록 높음), scalp_pore_score(모공 깨끗할수록 높음), scalp_hair_thickness_score(굵고 풍성할수록 높음)
...
def show_result(result, air, region, residence_years_str,
    ...
    st.markdown("<div class='patent-banner'>🔐 본 기술은 특허 출원 중입니다 — 환경오염 연동 AI 피부·두피 분석 및 화장품 제안 시스템</div>",
                unsafe_allow_html=True)
    ...
    if is_scalp:
        ...
        for i, (lbl, val, desc) in enumerate([
            ...
            with s_cols[i]:
                ...
                """, unsafe_allow_html=True)
        if sc_comment:
            st.markdown(f"<div style='font-size:0.83rem;color:#2e7d32;margin-top:0.7rem;"
    ...
    with col1:
        ...
        st.download_button("📄 분석 리포트 다운로드", data=pdf_html.encode("utf-8"),
                           file_name=f"YDLab_리포트_{datetime.now().strftime('%Y%m%d_%H%M')}.html",
                           mime="text/html", use_container_width=True, key="btn_dl_report")
    with col2:
        order_html = generate_order_html(result, air, region, yrs, name, age_group, gender)
        st.download_button("🧪 공방 주문서 다운로드", data=order_html.encode("utf-8"),
                           file_name=f"YDLab_주문서_{datetime.now().strftime('%Y%m%d_%H%M')}.html",
                           mime="text/html", use_container_width=True, key="btn_dl_order")
...
def generate_order_html(result, air, region, residence_years, name, age_group, gender):
    ...
    for i,(lbl,score) in enumerate(priority[:3]):
        color = "#c62828" if score<40 else "#e65100" if score<60 else "#2e7d32"
        priority_rows += f"<tr><td style='padding:8px;border:1px solid #e4e8ee;font-weight:600;color:#0f3460;'>{i+1}순위</td><td style='padding:8px;border:1px solid #e4e8ee;'>{lbl}</td><td style='padding:8px;border:1px solid #e4e8ee;font-weight:700;color:{color};'>{score}점</td><td style='padding:8px;border:1px solid #e4e8ee;color:#555;'>{'집중 케어 필요' if score<40 else '개선 권장' if score<60 else '유지 관리'}</td></tr>"
    ...
    for i,ing in enumerate(ingredients):
        ...
        ing_rows += f"<tr style='background:{bg}'><td style='padding:8px;border:1px solid #e4e8ee;text-align:center;font-weight:600;color:#0f3460;'>{i+1}</td><td style='padding:8px;border:1px solid #e4e8ee;font-weight:700;'>{ing}</td><td style='padding:8px;border:1px solid #e4e8ee;color:#555;'>{purpose}</td><td style='padding:8px;border:1px solid #e4e8ee;font-weight:600;color:#0f3460;font-family:monospace;'>{conc}</td><td style='padding:8px;border:1px solid #e4e8ee;color:#888;font-size:9px;'>{note}</td></tr>"

    scalp_section = ""
    if is_scalp:
        sk=result.get("scalp_keratin_score","-"); sp=result.get("scalp_pore_score","-")
        sh=result.get("scalp_hair_thickness_score","-"); sc_cmt=result.get("scalp_comment","")
        scalp_section = f"""<div class="section"><div class="stitle">두피 분석 결과 (3지표)</div>
        <div class="score-row">
...
.snum{{font-size:22px;font-weight:700;line-height:1;}}.slbl{{font-size:9px;color:#666;margin-top:2px;}}
.alert{{background:#fff3e0;border-left:3px solid #e65100;padding:8px 12px;font-size:10px;color:#e65100;margin-top:8px;}}
...
<div class="header"><div><h1>🧪 YD Lab 공방 주문서</h1>
...
<div class="score-row">{"".join([f'<div class="sbox"><div class="snum">{v}</div><div class="slbl">{k}</div></div>' for k,v in list(scores.items())[:5]])}</div>
...
<table><tr><th>순위</th><th>지표</th><th>점수</th><th>케어 방향</th></tr>{priority_rows}</table></div>
<div class="section"><div class="stitle">AI 추천 화장품 성분</div>
<table><tr><th style='width:5%'>#</th><th style='width:18%'>성분명</th><th style='width:30%'>목적</th><th style='width:15%'>권장 농도</th><th>제조 참고사항</th></tr>{ing_rows}</table></div>
<div class="section"><div class="stitle">환경 요인 (CEEI)</div>
<div style='font-size:10px;color:#555;margin-bottom:6px;'>PM2.5 {pm25}㎍/m³ ({station}) · CEEI {ceei} [{ceei_grade}] · {region} 거주 {residence_years}년 · 연평균 {pm25_avg}㎍/m³</div>
...
def generate_pdf_html(result, air, region, residence_years, name, age_group, gender):
    ...
    if is_scalp:
        sk=result.get("scalp_keratin_score","-"); sp=result.get("scalp_pore_score","-")
        sh=result.get("scalp_hair_thickness_score","-"); sc_cmt=result.get("scalp_comment","")
        scalp_section = f"""<div class="section"><div class="stitle">두피 분석 (3지표)</div>
        <div class="score-row">
          <div class="sbox" style="background:#f0faf4;border-color:#a5d6a7;"><div class="snum">{sk}</div><div class="slbl">각질 상태</div></div>
          <div class="sbox" style="background:#f0faf4;border-color:#a5d6a7;"><div class="snum">{sp}</div><div class="slbl">모공·피지</div></div>
          <div class="sbox" style="background:#f0faf4;border-color:#a5d6a7;"><div class="snum">{sh}</div><div class="slbl">모발 굵기</div></div>
        </div>
        <div style='font-size:9px;color:#2e7d32;padding:5px 8px;background:#e8f5e9;border-radius:4px;margin-top:5px;'>{sc_cmt}</div></div>"""
...
<div class="header"><div><h1>🔬 YD Lab 피부·두피 분석 리포트</h1>
<div class="sub">재능대학교 바이오테크과 · AI 바이오분석특화연구소 · 특허 출원 중</div></div>
<div class="date">{datetime.now().strftime("%Y년 %m월 %d일")}</div></div>
<div class="body">
<div class="infobar">
  <span>이름: {name}</span><span>연령대: {age_group}</span>
  <span>성별: {gender}</span><span>거주지: {region}</span><span>거주기간: {residence_years}년</span>
</div>
<div class="section"><div class="stitle">종합 결과</div>
<div class="overall-row"><div class="big-score">{overall}</div>
<div><div class="skin-type">피부 타입: {skin_type}</div><div class="summary">{summary}</div></div></div></div>
...
<div class="ing-row">{ing_html}</div><div class="care">{care}</div></div>
<div class="section"><div class="stitle">환경 지수 (실시간)</div>
<div class="chip-row">
  <span class="chip">PM2.5 {pm25}㎍/m³</span><span class="chip">PM10 {pm10}㎍/m³</span>
  <span class="chip">O₃ {o3}ppm</span><span class="chip">NO₂ {no2}ppm</span>
</div></div>
<div class="section"><div class="stitle">CEEI 누적 환경 노출 지수</div>
<div class="chip-row">
  <span class="chip" style="background:#{'fce4ec' if ceei>=300 else 'fff3e0' if ceei>=150 else 'e8f5e9'};color:{gc};">CEEI {ceei} [{ceei_grade}]</span>
  <span class="chip">지역 연평균 {pm25_avg}㎍/m³</span><span class="chip">거주 {residence_years}년</span>
</div>
<div class="small">{ceei_msg}</div></div>
</div>
<div class="footer">
  <span>본 리포트는 연구 참고용이며 의료적 진단을 대체하지 않습니다. 특허 출원 중.</span>
  <div style="text-align:right;"><div>YD Lab · 재능대학교 바이오테크과</div>
  <div style="opacity:0.7;margin-top:2px;">개발: 남정훈 교수 · Dept. of Biotechnology, Jaeneung Univ.</div></div>
</div></body></html>"""
...
| KAIST | 초미세먼지가 피부 장벽 기능(TEWL) 저하 유발 | Roberts, 2021 |
| 인하대학교 | 인천 지역 PM2.5 노출과 피부 조기 노화 상관관계 | 인하대 연구팀, 2024 |
| 미래과학연구원 | PM10 노출 시 모낭 내 β-카테닌 감소로 탈모 촉진 | FSRC, 2023 |

**CEEI 공식:** CEEI = PM2.5 지역연평균 × 거주년수
        """)
    ...
    with st.expander("📷 촬영 가이드", expanded=False):
        ...
        <div class='guide-body'>
          ...
          <div class='gstep'><span class='gnum'>3</span><span>피부·모공이 선명하게 보이도록 초점을 맞추세요.</span></div>
