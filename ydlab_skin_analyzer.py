...
.scalp-title { font-size:0.82rem; font-weight:700; color:#2e7d32; margin-bottom:0.8rem; }
</style>
""", unsafe_allow_html=True)

# ── 상수 ────────────────────────────────────────────────────
PM25_ALERT_THRESHOLD      = 35   # µg/m³ 이상 → 경보
CEEI_ANTIOXIDANT_THRESHOLD = 150 # CEEI 이상 → 항산화 강화 권장
...
def get_concentration(ingredient: str, skin_type: str) -> tuple:
    """피부 타입별 성분 권장 농도 반환 (농도, 참고사항)"""
...
def get_pollution_alert(pm25, ceei: float) -> str:
    """오염 경보 문자열 반환"""
...
def calc_ceei(pm25_avg: float, residence_years: int) -> tuple:
    """
    CEEI (Cutaneous Environmental Exposure Index) 계산
    공식: CEEI = PM2.5_지역연평균 × 거주년수
    (특허 출원 중 - 대규모 언어모델 기반 비전 AI 피부분석 시스템)
    반환: (ceei값, 등급, 칩HTML, 메시지)
    """
    ceei = round(pm25_avg * residence_years, 1)
    if ceei < 50:
        grade = "낮음"
        chip  = f"<span class='chip chip-good'>CEEI {ceei} [낮음]</span>"
        msg   = "환경 노출 영향 낮음 — 기본 보습·자외선 차단 유지"
    elif ceei < 150:
        grade = "보통"
        chip  = f"<span class='chip chip-mid'>CEEI {ceei} [보통]</span>"
        msg   = "중간 수준 환경 노출 — 항산화 성분 정기적 사용 권장"
    elif ceei < 300:
        grade = "높음"
        chip  = f"<span class='chip chip-warn'>CEEI {ceei} [높음]</span>"
        msg   = "높은 환경 노출 누적 — 항산화·장벽강화 집중 케어 필요"
    else:
        grade = "매우높음"
        chip  = f"<span class='chip chip-bad'>CEEI {ceei} [매우높음]</span>"
        msg   = "매우 높은 누적 노출 — 피부과 상담 및 기능성 화장품 집중 케어 권장"
    return (ceei, grade, chip, msg)
...
def fetch_air(region: str) -> dict:
    ...
    if key:
        for station in candidates:
            try:
                params = dict(
                    serviceKey=key, stationName=station,
...

{
  ...
  "scalp_keratin_score": 두피사진이면 0~100 / 얼굴이면 null,
  "scalp_pore_score": 두피사진이면 0~100 / 얼굴이면 null,
  "scalp_hair_thickness_score": 두피사진이면 0~100 / 얼굴이면 null,
  "scalp_comment": "두피 종합 상태 설명 (50자 이내)" 또는 null
}

점수 기준: 높을수록 좋음 (100=최상, 0=매우불량)
두피 판별 기준: 모발·모공·각질이 보이면 is_scalp=true로 판단
두피 3지표:
  - scalp_keratin_score: 각질 분포 및 두께 (100=각질 없음, 0=심한 각질)
  - scalp_pore_score: 두피 모공 상태·피지 분비 (100=깨끗한 모공, 0=막힌 모공)
  - scalp_hair_thickness_score: 모발 굵기·밀도 (100=굵고 풍성, 0=매우 가늘고 적음)
사진이 불명확해도 최대한 추정하여 응답하세요.
"""

# ── LLM 비전 분석 ────────────────────────────────────────────
def analyze_skin(images: list, api_key: str, body_parts: list = None) -> dict | None:
    """
    LLM 기반 비전 AI (Claude) 피부·두피 분석
    특허 출원 중 - 환경오염 연동 AI 피부·두피 분석 및 화장품 제안 시스템
    """
    try:
        ...
        parts_str = ""
        if body_parts:
            parts_str = f"\n\n[촬영 부위 정보]: {', '.join(body_parts)}\n위 부위에 맞게 분석을 최적화하세요."
            if any("두피" in p for p in body_parts):
                parts_str += "\n두피 부위가 포함되어 있으므로 is_scalp=true로 설정하고 두피 3지표를 반드시 채우세요."

        content.append({"type": "text", "text": ANALYSIS_PROMPT + parts_str})

        # LLM 기반 비전 AI 호출 (특허 명세서 상 "대규모 언어모델 기반 비전 AI"에 해당)
...
FIELDS = [
    "timestamp", "name", "age_group", "gender", "region",
    "residence_years", "skin_concern", "body_parts", "photo_count",
    "pm25", "pm10", "o3", "no2",
...
# ── 결과 표시 ────────────────────────────────────────────────
def show_result(result: dict, air: dict, region: str,
                residence_years_str: str, name: str,
                age_group: str, gender: str, selected_parts: list):
    ...
    # ── 특허 배너
    st.markdown("<div class='patent-banner'>🔐 본 기술은 특허 출원 중입니다 — 환경오염 연동 AI 피부·두피 분석 및 화장품 제안 시스템</div>", unsafe_allow_html=True)
    ...
    <div class='card'>
      ...
      <div style='display:flex;align-items:center;gap:1.5rem;flex-wrap:wrap;'>
        <div style='text-align:center;'>
          <div style='font-size:3.5rem;font-weight:700;color:{sc(overall)};line-height:1;font-family:"DM Mono",monospace;'>{overall}</div>
    ...
    if is_scalp:
        ...
        st.markdown("<div class='scalp-card'><div class='scalp-title'>🌿 두피 분석 결과 (3지표)</div>", unsafe_allow_html=True)
        s_cols = st.columns(3)
        scalp_metrics = [
            ("각질 상태", sk, "각질 분포·두께"),
        ...
        for i, (lbl, val, desc) in enumerate(scalp_metrics):
            with s_cols[i]:
                v = val if val is not None else 0
                st.markdown(f"""
                <div class='score-box' style='background:#f0faf4;border-color:#a5d6a7;'>
                  <div class='score-num' style='color:{sc(v)};'>{v if val is not None else "-"}</div>
                  <div class='score-lbl'>{lbl}</div>
                  <div style='font-size:0.68rem;color:#999;margin-top:0.3rem;'>{desc}</div>
                </div>
                """, unsafe_allow_html=True)
        if sc_comment:
            st.markdown(f"<div style='font-size:0.83rem;color:#2e7d32;margin-top:0.7rem;padding:0.6rem;background:#e8f5e9;border-radius:6px;'>{sc_comment}</div>", unsafe_allow_html=True)
    ...
    if is_scalp and result.get("scalp_keratin_score") is not None:
    ...
    for i, (lbl, score) in enumerate(priority):
        ...
        priority_html += f"""
        <div style='display:flex;align-items:center;gap:0.8rem;padding:0.6rem 0;border-bottom:1px solid #f0f0f0;'>
          ...
          <span style='font-size:0.78rem;color:#888;'>{"집중 케어 필요" if score<40 else "개선 권장" if score<60 else "유지 관리"}</span>
        </div>"""
    st.markdown(f"""
    <div class='card'>
...
def generate_order_html(result: dict, air: dict, region: str,
    ...
    code = "YDL-" + _dt.now().strftime("%Y%m%d") + "-" + ''.join(
    ...
    for i, ing in enumerate(ingredients):
        ...
        <tr style='background:{"#f8faff" if i%2==0 else "white"}'>
          ...
          <td style='padding:8px;border:1px solid #e4e8ee;font-weight:700;'>{ing}</td>
    ...
    if is_scalp:
        ...
        scalp_section = f"""
  <div class="section">
    <div class="stitle">두피 분석 결과 (3지표)</div>
    <div class="score-row">
      <div class="sbox"><div class="snum">{sk}</div><div class="slbl">각질 상태</div></div>
      <div class="sbox"><div class="snum">{sp}</div><div class="slbl">모공·피지</div></div>
      <div class="sbox"><div class="snum">{sh}</div><div class="slbl">모발 굵기</div></div>
    </div>
    <div style='font-size:10px;color:#2e7d32;padding:6px 8px;background:#e8f5e9;border-radius:4px;margin-top:6px;'>{sc_cmt}</div>
  </div>"""
...
.sbox{{flex:1;min-width:70px;background:#f8f9fa;border:1px solid #e4e8ee;border-radius:6px;padding:8px;text-align:center;}}
...
<div class="header">
  <div>
    <h1>🧪 YD Lab 공방 주문서</h1>
    <div class="sub">AI 피부·두피 분석 기반 맞춤형 화장품 제조 요청 · 재능대학교 AI-바이오분석특화연구소</div>
  </div>
...
<div class="body">
  ...
  {scalp_section}
  <div class="section">
    <div class="stitle">우선 개선 항목</div>
    <table>
      <tr><th>순위</th><th>지표</th><th>점수</th><th>케어 방향</th></tr>
      {priority_rows}
    </table>
  </div>
  <div class="section">
    ...
    <table>
      <tr><th style='width:5%'>#</th><th style='width:18%'>성분명</th><th style='width:30%'>목적</th><th style='width:15%'>권장 농도</th><th>제조 참고사항</th></tr>
      {ing_rows}
    </table>
  </div>
  <div class="section">
    <div class="stitle">환경 요인 (CEEI 누적 노출 지수)</div>
  ...
  <div class="section">
    <div class="stitle">생활 관리 조언</div>
    <div style='font-size:10px;color:#555;line-height:1.7;'>{care}</div>
  </div>
</div>
<div class="footer">
  <span>본 주문서는 YD Lab AI 분석 결과 기반이며 의료적 처방이 아닙니다. 특허 출원 중.</span>
...
def generate_pdf_html(result: dict, air: dict, region: str,
    ...
    if is_scalp:
        ...
        scalp_section = f"""
  <div class="section">
    <div class="stitle">두피 분석 (3지표)</div>
    <div class="score-row">
      <div class="sbox" style="background:#f0faf4;border-color:#a5d6a7;">
        <div class="snum" style="color:{sc(sk if isinstance(sk,int) else 0)}">{sk}</div>
        <div class="slbl">각질 상태</div>
      </div>
      <div class="sbox" style="background:#f0faf4;border-color:#a5d6a7;">
        <div class="snum" style="color:{sc(sp if isinstance(sp,int) else 0)}">{sp}</div>
        <div class="slbl">모공·피지</div>
      </div>
      <div class="sbox" style="background:#f0faf4;border-color:#a5d6a7;">
        <div class="snum" style="color:{sc(sh if isinstance(sh,int) else 0)}">{sh}</div>
        <div class="slbl">모발 굵기</div>
      </div>
    </div>
    <div style='font-size:9px;color:#2e7d32;padding:5px 8px;background:#e8f5e9;border-radius:4px;margin-top:5px;'>{sc_cmt}</div>
  </div>"""
...
<div class="body">
  ...
  <div class="section">
    <div class="stitle">세부 지표 (피부 5지표)</div>
  ...
  <div class="section">
    <div class="stitle">환경 지수 (실시간)</div>
    <div class="chip-row">
  ...
  <div class="section">
    <div class="stitle">CEEI 누적 환경 노출 지수</div>
    <div class="chip-row">
      <span class="chip" style="background:#{"fce4ec" if ceei>=300 else "fff3e0" if ceei>=150 else "e8f5e9"};color:{gc};">
        CEEI {ceei} [{ceei_grade}]
      </span>
      <span class="chip">지역 연평균 {pm25_avg}㎍/m³</span>
      <span class="chip">거주 {residence_years}년</span>
    </div>
    <div class="small">{ceei_msg}</div>
  </div>
</div>
<div class="footer">
...
def main():
    ...
    if not api_key:
        st.error("⚠️ ANTHROPIC_API_KEY가 설정되지 않았습니다. Streamlit Secrets를 확인하세요.")
        st.stop()
    ...
    with st.expander("📚 연구 배경 및 과학적 근거", expanded=False):
        st.markdown("""
        **PM2.5와 피부·두피의 과학적 연관성**

        | 연구 기관 | 주요 발견 | 출처 |
        |---|---|---|
        | 서울대학교 의과대학 | PM2.5 10㎍/m³ 증가 시 피부 수분도 12% 감소 | Han et al., 2025 |
        | KAIST 나노과학기술대학원 | 초미세먼지가 피부 장벽 기능(TEWL) 저하 유발 | Roberts, 2021 |
        | 인하대학교 환경의학연구소 | 인천 지역 PM2.5 노출과 피부 조기 노화 상관관계 확인 | 인하대 연구팀, 2024 |
        | 미래과학연구원 | PM10 노출 시 모낭 내 β-카테닌 감소로 탈모 촉진 | FSRC, 2023 |
        ...
        **CEEI (Cutaneous Environmental Exposure Index)**
        ...
        - CEEI < 50: 환경 노출 낮음
        - 50 ≤ CEEI < 150: 보통
        - 150 ≤ CEEI < 300: 높음 (항산화 집중 케어 권장)
        - CEEI ≥ 300: 매우높음 (피부과 상담 권장)
        """)
    ...
    with st.expander("📷 촬영 가이드", expanded=False):
        ...
        <div class='guide-body'>
          <div class='gstep'><span class='gnum'>1</span><span>스마트폰에 클립온 현미경(50~200배)을 장착하세요.</span></div>
          <div class='gstep'><span class='gnum'>2</span><span>분석할 부위(이마·볼·눈가·두피 등)를 선택하고 밝은 곳에서 촬영하세요.</span></div>
          <div class='gstep'><span class='gnum'>3</span><span>피부·모공이 선명하게 보이도록 초점을 맞추세요 (흔들리지 않게 고정).</span></div>
          <div class='gstep'><span class='gnum'>4</span><span>두피 촬영 시: 모발을 가르마로 나누어 두피 표면이 잘 보이도록 하세요.</span></div>
          <div class='gstep'><span class='gnum'>5</span><span>촬영 후 아래 양식을 작성하고 사진을 업로드하세요.</span></div>
        </div>
        """, unsafe_allow_html=True)
    ...
    with col1:
        name = st.text_input("이름 (익명 가능)", value="익명", placeholder="홍길동")
    with col2:
        age_group = st.selectbox("연령대", ["선택", "10대", "20대", "30대", "40대", "50대", "60대 이상"])
    with col3:
        gender = st.selectbox("성별", ["선택", "여성", "남성", "기타"])

    col4, col5 = st.columns(2)
    with col4:
        region = st.selectbox("거주 지역", list(REGION_PM25_AVG.keys()))
    with col5:
        residence_years_str = st.selectbox("거주 기간", list(RESIDENCE_YEAR_MAP.keys()))
    ...
    # ── 촬영 부위 선택 (발명신고서 청구범위 반영)
    st.markdown("</div>", unsafe_allow_html=True)
    st.markdown("<div class='card'><div class='card-label'>촬영 부위 선택</div>", unsafe_allow_html=True)

    selected_parts = st.multiselect(
        "📍 촬영 부위를 선택하세요 (복수 선택 가능)",
        BODY_PARTS,
        default=["볼"],
        help="선택한 부위에 맞게 AI 분석이 최적화됩니다. 두피 선택 시 두피 3지표(각질·모공·모발굵기)가 함께 분석됩니다."
    )

    if any("두피" in p for p in selected_parts):
        st.info("💡 두피 부위가 선택되었습니다. 두피 각질·모공·모발 굵기 3지표가 추가 분석됩니다.")

    st.markdown("</div>", unsafe_allow_html=True)

    # ── 사진 업로드
    st.markdown("<div class='card'><div class='card-label'>피부·두피 사진 업로드</div>", unsafe_allow_html=True)
    uploaded = st.file_uploader(
        "현미경 촬영 사진을 업로드하세요 (최대 3장)",
        type=["jpg", "jpeg", "png"],
    ...
    research_consent = st.checkbox(
        "🔬 [선택] 연구 참여 동의 (IRB 면제심의 대상): 익명화된 피부 이미지 및 환경 데이터를 학술 연구에 활용하는 것에 동의합니다."
    )
    st.markdown("</div>", unsafe_allow_html=True)

    # ── 분석 버튼
    st.markdown("<br>", unsafe_allow_html=True)
    run = st.button("🔬 AI 피부·두피 분석 시작", use_container_width=True, type="primary")
    ...
    if run:
        ...
        with st.spinner("🌡️ 실시간 대기오염 데이터 수집 중..."):
            ...
            if air.get("mock"):
                st.caption("⚠️ AirKorea API 연결 실패 — 참고용 Mock 데이터 사용 중")
        ...
        if admin_pw == st.secrets.get("ADMIN_PASSWORD", "ydlab2024"):
            ...
            if DATA_FILE.exists():
                ...
                if len(df) > 0:
                    ...
                    st.markdown("**두피 분석 건수**")
                    if "is_scalp" in df.columns:
                        scalp_count = df["is_scalp"].sum()
                        st.metric("두피 분석", f"{scalp_count}건")
